namespace DotNetty.Buffers.Tests
{
    ...
    public abstract partial class AbstractArrayPooledByteBufferTests : IDisposable
    {
        ...
        protected virtual void SetCharSequenceNoExpand(Encoding encoding)
        {
            IByteBuffer buf = this.NewBuffer(1);
            try
            {
        ...
        public void CapacityEnforceMaxCapacity()
        {
            ...
            IByteBuffer buf = this.NewBuffer(180, 1024);
            Assert.Equal(1024, buf.MaxCapacity);
            Assert.Equal(256, buf.Capacity);
            try
            {
                Assert.Throws<ArgumentOutOfRangeException>(() => buf.AdjustCapacity(2048));
            }
            finally
            {
                buf.Release();
