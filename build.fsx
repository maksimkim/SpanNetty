    ...
    if not skipBuild.Value then
        ...
        let runSingleProject project =
            let arguments =
                ...
                | false -> (sprintf "test -c %s --no-build --logger:trx --logger:\"console;verbosity=detailed\" --framework %s -- RunConfiguration.TargetPlatform=x64 --results-directory \"%s\" --filter FullyQualifiedName~ConnectionRoundtripTest -- -parallel none" configuration testNetVersion outputTests)
